...
// SETUP
	...
	require_once 'instanceCtl.config.php';
...
// THE GOALS & PREPARATION
	...
	*/
	...
	$ec2 = new AmazonEC2(array(
							"default_cache_config" => "c:\tmp",
							"key"=>$ACCKEY[$keyid]['key'],
							"secret"=>$ACCKEY[$keyid]['sec']
							)
						);
	$regs = $ec2->describe_regions();
	$instances = array();
	
	foreach($regs->body->regionInfo->item as $rg)
	{
		$ec2->set_region((string)$rg->regionEndpoint);
		...
		foreach ($response->body->reservationSet->item as $item)
		{
			...
			foreach($item->instancesSet->item as $item2)
			{
				...
				array_push($instances,
					array('region'=>(string)$rg->regionEndpoint
					,'instanceId'=>(string)$item2->instanceId
					,'instanceState'=>$state
